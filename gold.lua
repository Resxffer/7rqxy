-- OBSIDIAN & GOLD UI Library v4.0 | Mobile Optimized
--
-- A from-scratch re-theme, not just a recolor: this replaces every place the
-- old kit reached for a purple glow, a blue-to-purple gradient, or a diagonal
-- "glass" sheen, and rebuilds those spots around one idea instead — dark
-- lacquered obsidian panels with a single hairline of satin gold.
--
-- Changes from the previous "Midnight Glass" build:
--   * New palette: obsidian-black base, warm ivory text, one accent color
--     (satin gold, with a brighter gold for hover/active states) instead of
--     a purple/blue two-stop gradient. Every gradient fill in the old kit
--     (toggle fill, slider fill, tab underline, divider) is now a flat
--     solid color — closer to lacquer and metal than to glass.
--   * Panels are now mostly opaque ("lacquered", ~90-96% opaque) instead of
--     translucent frosted glass, which reads calmer at small sizes; the
--     backdrop blur behind the whole window is kept, since that's what
--     gives the depth — not fog on every panel.
--   * The diagonal glass-sheen overlay and the fading rim-light gradient
--     are both gone, replaced by a single flat gold hairline (a UIStroke)
--     and, on the main window only, a flat 1px top edge.
--   * Corner radii pulled in (18 -> 10 on the window, 12 -> 7 on rows) for
--     a crisper, more architectural shape.
--   * The title uses a serif face (Garamond) in gold instead of the default
--     Gotham, so the brand name itself carries the identity — no logo
--     asset needed. Set Config.LibraryName (or Config.Title) to your own
--     name; nothing else in the file needs to change, and the floating
--     mobile toggle button picks up its initial automatically.
--   * The active tab is now a filled gold pill with dark text instead of a
--     faint outline, so the current tab reads at a glance.
--   * Slider thumb gets a thin gold ring instead of being a plain white dot.
--   * Still built on the same tested internals as before: safe CoreGui/
--     PlayerGui fallback, clamped dragging, leak-free Destroy, the fixed-
--     pixel dropdown (sized independently of the animating row so nothing
--     goes negative mid-tween), and only one dropdown open at a time.
...
local DefaultTheme = {
	Background = Color3.fromRGB(12, 11, 10),           -- obsidian base
	Panel = Color3.fromRGB(22, 20, 17),                 -- lacquered card
	Accent = Color3.fromRGB(196, 160, 84),              -- satin gold
	AccentBright = Color3.fromRGB(230, 199, 130),       -- hover / active gold
	Text = Color3.fromRGB(237, 230, 214),               -- warm ivory
	SubText = Color3.fromRGB(150, 141, 123),            -- warm ash
	SectionHeader = Color3.fromRGB(196, 160, 84),       -- gold section labels
	Success = Color3.fromRGB(137, 189, 150),            -- jade
	Error = Color3.fromRGB(191, 89, 74),                -- garnet
	BackgroundTransparency = 0.04,   -- near-opaque; not frosted glass
	PanelTransparency = 0.10,
	StrokeTransparency = 0.35,
	CornerRadius = 10,       -- main window
	RowCornerRadius = 7,     -- card rows
...
-- A single flat gold hairline. No gradient sheen — that's the whole point
-- of the obsidian-and-gold look: one clean line of metal, not a shimmer.
local function addHairline(frame, theme, transparency)
...
-- A flat 1px gold edge along the very top of the main window only — the
-- trim line on the lid of the box. Used once, not on every row, so it
-- reads as a deliberate detail rather than a texture.
local function addTopEdge(frame, theme, transparency)
	local edge = Instance.new("Frame")
	edge.Name = "TopEdge"
	edge.BackgroundColor3 = theme.Accent
	edge.BorderSizePixel = 0
	edge.BackgroundTransparency = transparency or 0.55
	edge.AnchorPoint = Vector2.new(0.5, 0)
	edge.Position = UDim2.new(0.5, 0, 0, 1)
	edge.Size = UDim2.new(1, -8, 0, 1)
	edge.ZIndex = (frame.ZIndex or 1) + 1
	edge.Parent = frame
	return edge
end

-- Creates an opaque lacquered card: dark background + rounded corners +
-- a single gold hairline. No gradient overlay.
local function newPanel(parent, theme, size, position, cornerRadius)
	...
	f.BorderSizePixel = 0
	Instance.new("UICorner", f).CornerRadius = UDim.new(0, cornerRadius or theme.RowCornerRadius or 7)
	local stroke = addHairline(f, theme, theme.StrokeTransparency)
	return f, stroke
end
...
function Library:CreateWindow(config)
	...
	local BrandName = config.LibraryName or "OBSIDIAN"
	local TitleText = config.Title or BrandName
	...
	if UseBlur then
		...
		BlurEffect.Name = BrandName .. "_Blur_" .. tostring(math.random(1, 999999))
		BlurEffect.Size = 0
	...
	ToggleBtn.Text = BrandName:sub(1, 1):upper()
	ToggleBtn.TextColor3 = Theme.Accent
	ToggleBtn.Font = Enum.Font.GothamBold
	ToggleBtn.TextSize = 20
	Instance.new("UICorner", ToggleBtn).CornerRadius = UDim.new(1, 0)
	addShadow(ToggleBtn, 0.6, 14)
	addHairline(ToggleBtn, Theme, 0.25)
	...
	Main.Visible = false
	Instance.new("UICorner", Main).CornerRadius = UDim.new(0, Theme.CornerRadius or 10)
	addShadow(Main, 0.5, 30)
	addHairline(Main, Theme, 0.3)
	addTopEdge(Main, Theme, 0.5)

	-- Open/close as a soft scale+fade pop instead of an instant Visible flip
	...
	local function setOpen(state)
		...
		if state then
			...
			tween(Main, easeInfo(0.22), {BackgroundTransparency = Theme.BackgroundTransparency})
			if BlurEffect then tween(BlurEffect, easeInfo(0.25), {Size = BlurSize}) end
		else
			tween(MainScale, easeInfo(0.16, Enum.EasingStyle.Quint, Enum.EasingDirection.In), {Scale = 0.94})
			tween(Main, easeInfo(0.16), {BackgroundTransparency = 1})
			if BlurEffect then tween(BlurEffect, easeInfo(0.2), {Size = 0}) end
	...
	Title.Font = Theme.TitleFont or Enum.Font.Garamond
	Title.TextSize = 17
	...
	-- Notifications (stacked bottom-right, obsidian cards)
	...
	local Tabs = {}
	local FirstTab = true
	local ActiveDropdownClose = nil -- only one dropdown open across the whole window at a time
	...
	function Window:CreateTab(name)
		...
		TabBtn.BackgroundColor3 = selected and Theme.Accent or Theme.Panel
		TabBtn.BackgroundTransparency = selected and 0.15 or 0.92
		TabBtn.BorderSizePixel = 0
		TabBtn.AutoButtonColor = false
		TabBtn.Text = name
		TabBtn.TextColor3 = selected and Theme.Background or Theme.SubText
		...
		TabStroke.Transparency = selected and 0.2 or 0.92
		...
		track(TabBtn.MouseButton1Click:Connect(function()
			for _, t in pairs(Tabs) do
				...
				tween(t.Btn, easeInfo(0.15), {BackgroundColor3 = Theme.Panel, BackgroundTransparency = 0.92})
				tween(t.Stroke, easeInfo(0.15), {Transparency = 0.92})
				t.Btn.TextColor3 = Theme.SubText
			end
			TabPage.Visible = true
			tween(TabBtn, easeInfo(0.15), {BackgroundColor3 = Theme.Accent, BackgroundTransparency = 0.15})
			tween(TabStroke, easeInfo(0.15), {Transparency = 0.2})
		...
		function Tab:CreateDropdown(opts)
			...
			-- Fixed-pixel list container: sized independently of Row so nothing
			-- goes negative while Row is mid-tween.
			...
			ListHolder.BackgroundTransparency = 0.05
			...
			function Drop:Set(option, fire)
				...
				for _, c in ipairs(List:GetChildren()) do
					if c:IsA("TextButton") then
						...
						tween(c, easeInfo(0.15), {BackgroundTransparency = sel and 0.75 or 1})
			...
			function Drop:Refresh(options)
				...
				for _, itm in ipairs(options) do
					...
					ib.BackgroundTransparency = selected and 0.75 or 1
	...
	function Window:Notify(opts)
		...
		tween(N, easeInfo(0.2), {BackgroundTransparency = Theme.PanelTransparency})
