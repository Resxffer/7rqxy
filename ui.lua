...
local DefaultTheme = {
	Background = Color3.fromRGB(9, 9, 13),            -- near-black glass base
	Panel = Color3.fromRGB(22, 22, 30),                -- dark elevated glass rows
	...
	CornerRadius = 18,      -- main window
	RowCornerRadius = 12,   -- glass row cards
}
...
-- Smooth "premium" easing curve used everywhere by default instead of a
-- bare TweenInfo.new(duration), which defaults to a flat Linear feel.
...
-- Thin light-catching line along the very top edge — the small detail that
-- reads as "glass" rather than "flat dark rectangle".
local function addRimLight(frame)
	...
	rim.Size = UDim2.new(1, -8, 0, 1)
	rim.ZIndex = (frame.ZIndex or 1) + 1
	local grad = Instance.new("UIGradient", rim)
	grad.Color = ColorSequence.new(Color3.new(1, 1, 1), Color3.new(1, 1, 1))
	grad.Transparency = NumberSequence.new({
...
-- Creates a translucent "glass card" row: dark background + rounded
-- corners + sheen.
local function newGlassPanel(parent, theme, size, position, cornerRadius)
	...
	f.BorderSizePixel = 0
	Instance.new("UICorner", f).CornerRadius = UDim.new(0, cornerRadius or theme.RowCornerRadius or 10)
...
function Library:CreateWindow(config)
	...
	Main.Visible = false
	Instance.new("UICorner", Main).CornerRadius = UDim.new(0, Theme.CornerRadius or 18)
	addShadow(Main, 0.5, 30)
	addGlassSheen(Main, 0.4, Theme)
	addRimLight(Main)
	...
	function Window:CreateTab(name)
		...
		TabBtn.BackgroundTransparency = selected and 0.3 or 0.92
		...
		TabStroke.Transparency = selected and 0.35 or 0.92
		...
		track(TabBtn.MouseButton1Click:Connect(function()
			for _, t in pairs(Tabs) do
				...
				tween(t.Btn, easeInfo(0.15), {BackgroundTransparency = 0.92})
				tween(t.Stroke, easeInfo(0.15), {Transparency = 0.92})
				t.Btn.TextColor3 = Theme.SubText
			end
			TabPage.Visible = true
			tween(TabBtn, easeInfo(0.15), {BackgroundTransparency = 0.3})
			tween(TabStroke, easeInfo(0.15), {Transparency = 0.35})
		...
		function Tab:CreateToggle(opts)
			opts = opts or {}
			local Row = newGlassPanel(TabPage, Theme, UDim2.new(1, 0, 0, 42))
			...
			-- Sliding pill switch instead of a checkbox
		...
		function Tab:CreateSlider(opts)
			...
			if maxV == minV then maxV = minV + 1 end -- avoid divide-by-zero
		...
		function Tab:CreateDropdown(opts)
			...
			local Row = newGlassPanel(TabPage, Theme, UDim2.new(1, 0, 0, rowHeight))
			...
			-- Fixed-pixel list container. This is the actual v3.0 fix: nothing
			-- here is sized as a Scale of Row, so nothing goes negative while
			-- Row is mid-tween.
			...
			local close, openDrop -- forward-declared so each can reference the other
			...
			openDrop = function()
				...
				-- only one dropdown open at a time, so they can't overlap each other
				...
				-- auto-scroll the tab so the opened list is never clipped by the
				-- scroll container, regardless of how much room happens to be
				-- left below the row
				task.defer(function()
	...
	function Window:Notify(opts)
		...
		tween(N, easeInfo(0.2), {BackgroundTransparency = 0.1})
